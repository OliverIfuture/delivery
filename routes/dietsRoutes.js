...
module.exports = (app) => {
    ...
    app.post('/api/ai-diets/generate', passport.authenticate('jwt', { session: false }), upload.array('images', 3), aiDietController.generateDietJSON);

    // Paso 2: Guardar PDF (Recibe 1 archivo PDF)
    app.post('/api/ai-diets/upload-pdf', passport.authenticate('jwt', { session: false }), upload.single('pdf'), aiDietController.uploadDietPdf);
}
