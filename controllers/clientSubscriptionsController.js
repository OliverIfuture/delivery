    ...
    async cancelSubscription(req, res, next) {
        try {
            ...
            if (!stripe_subscription_id) {
                return res.status(400).json({
                    success: false,
                    message: 'ID de suscripción inválido o vacío.'
                });
            }

            // 🔥 1. DETECTAR SI ES MANUAL O DOMICILIADA (STRIPE) 🔥
            const isManual = stripe_subscription_id.startsWith('manual_');

            // Si NO es manual, nos comunicamos con Stripe para detener los cobros automáticos
            if (!isManual) {
                const User = require('../models/user.js');
                const company = await User.findCompanyById(id_company);
            ...
            // 🔥 LÓGICA COMPARTIDA (Para Manual y Stripe) 🔥
            const db = require('../config/config.js');
            const ClientSubscription = require('../models/clientSubscription.js');

            // 2. Actualizar la base de datos local (Estado de la suscripción)
            await ClientSubscription.updateStatus(stripe_subscription_id, 'canceled');

            // 3. ELIMINAR EL PAGO DEL MES CORRIENTE EN EL HISTORIAL
            try {
                const deleteQuery = `
            ...
            return res.status(200).json({
                success: true,
                message: isManual
                    ? 'Suscripción manual cancelada y registros depurados.'
                    : 'Suscripción domiciliada cancelada en Stripe y registros depurados.'
            });
...
*/
    ...
    async createSubscriptionIntent(req, res, next) {
        try {
            ...
            let subscriptionParams = {
                ...
                // Configuración para transferir dinero al entrenador
                transfer_data: {
                    destination: company.stripeAccountId,
                },
        ...
        try {
            ...
            const subscriptionConfig = {
                ...
                metadata: {
                    ...
                    id_company: id_company,
                    id_plan: id_plan,
                    duration_days: 30, // Default asegurado
                    temp_email: req.user.email.toLowerCase() // Protegido contra null
                },
                payment_settings: {
                    save_default_payment_method: 'on_subscription',
                    payment_method_options: {
            ...
            // Si logramos crear el cupón sin errores, se lo asignamos
            if (couponId) {
            ...
            console.log(`Error CRÍTICO en createSubscriptionIntentAutom: ${error}`);
            return res.status(501).json({
                success: false,
    ...
    async createPackagePaymentIntent(req, res) {
        try {
            ...
            // 2. Obtener el ID de la cuenta conectada de Stripe (acct_XXXXXXXXXX)
            ...
            if (!settings || !settings.stripe_account_id) {
                return res.status(400).json({
                    success: false,
                    message: 'El estudio aún no tiene configurado su ID de cuenta conectada de Stripe.'
                });
            }
